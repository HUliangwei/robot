# 🎯 pusht — PushT × LeRobot × MuJoCo（小项目介绍）
...
- CNN 编码器 + 高斯策略头（Actor，tanh 输出）+ 双 Critic 价值网络 + 自动熵调节
- 无专家演示：actor 在 gym-pusht 仿真里试错采集经验 → critic 学打分 → actor 学改进
- 归一化设计：gym-pusht 动作是 [0,512] 像素目标位置，用 action stats（min=256/max=512）对齐 tanh 输出 (-1,1)
...
| ACT（**自训**） | 本地训练 25k 步 | `outputs/train/act_pusht_real/checkpoints/025000/pretrained_model` | l1=0.12，与社区相当 |
| SAC（**自训**） | 本地训练 800 优化步 | `outputs/train/sac_pusht_smoke/checkpoints/000800/pretrained_model` | 冒烟验证管线（覆盖率 26.6%，欠训练） |
...
# SAC on PushT（HILSerl 双进程：learner 训练 + actor 交互）
python rl_scripts/run_sac_pusht.py --config_path rl_configs/sac_pusht_smoke.json --clean
```
...
# SAC checkpoint 评估（= SAC 的推理：确定性动作跑环境）
python rl_scripts/eval_sac_pusht.py --checkpoint outputs/train/sac_pusht_smoke/checkpoints/last \
