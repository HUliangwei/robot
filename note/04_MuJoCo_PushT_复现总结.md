...
- **「T 块地面太滑」假说测试（2026-08-17）**：❌ 不成立。诊断显示 MuJoCo 里 T 块被推后同样无摩擦滑动（速度恒定，与 pymunk 一致——pymunk 也无地面摩擦/阻尼）；给 T 块加滑动阻尼 0.3 后覆盖率反而降（0.50 vs 0.865），agent 质量 ×100 也变差（0.456）。0.865 上限是**接触求解器保真度**问题（pymunk 的刚性 kinematic 推动 vs MuJoCo 接触解算），非地面摩擦。
