...
class LocalExecutor:
    ...
    def run(self, job_id: str, command: CommandSpec) -> ExecutionResult:
        ...
        env = os.environ.copy(); env.update({str(k):str(v) for k,v in command.env.items()})
        cwd = Path(command.cwd).resolve() if command.cwd else self.project_root
        argv = command.normalized_argv()
        atomic_write_json(attempt_path, {"schema_version":"rlw.execution_attempt/v1","attempt_id":attempt_id,"job_id":job_id,"state":"RUNNING","argv":list(argv),"cwd":str(cwd),"started_at":started})
        ...
        atomic_write_json(attempt_path, {"schema_version":"rlw.execution_attempt/v1","attempt_id":attempt_id,"job_id":job_id,"state":state,"argv":list(argv),"cwd":str(cwd),"started_at":started,"ended_at":datetime.now(timezone.utc).isoformat(),"exit_code":completed.returncode})
        return ExecutionResult(job_id,attempt_id,state,completed.returncode,str(stdout_path),str(stderr_path),str(attempt_path))
